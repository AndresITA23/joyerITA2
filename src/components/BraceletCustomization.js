...
function BraceletCustomization({ route, addToCart }) {
  ...
  const colors = ['Negro', 'Rojo', 'Blanco', 'Azul', 'Verde', 'Rosa', 'Gris'];
  const materials = ['Hilo chino', 'Dije de metal'];
  const sizes = ['30 cm', '35 cm', '40 cm'];

  const handleSave = async () => {
    if (!materialType || !size || !threadColor ) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>1. Selecciona el tipo de material</Text>
        {renderRadioButtons(materials, materialType, setMaterialType)}
      </View>
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>3. Seleccionar color del hilo</Text>
...
export default BraceletCustomization;
